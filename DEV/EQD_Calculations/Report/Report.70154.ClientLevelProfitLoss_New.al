report 70154 "Client Level Profit & Loss"
{
    ...
    dataset
    {
        dataitem("Client Level Profit Loss"; "Client Level Profit Loss")
        {
            ...
            begin
                ...
                ExcelBuffer.AddColumn('NetP&L OnScenario1A', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
                ExcelBuffer.AddColumn('NetP&L OnScenario1B', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
                ExcelBuffer.AddColumn('NetP&L OnScenario2A', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
                ExcelBuffer.AddColumn('NetP&L OnScenario2B', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
                ExcelBuffer.AddColumn('NetP&L OnScenario3', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
                ExcelBuffer.AddColumn('NetP&L OnScenario4', FALSE, '', TRUE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
            ...
            begin
                ...
                ScenarioAnalysis.SetRange("Cm Code", "Client Level Profit Loss".CmCode);
                ScenarioAnalysis.SetRange("Tm Code", "Client Level Profit Loss".TmCode);
                ScenarioAnalysis.SetRange("Client Code", "Client Level Profit Loss".ClientCode);
                if ScenarioAnalysis.FindSet() then begin
                    repeat
                        SenPrice1A += ScenarioAnalysis."Profit/Loss on Scenario 1A";
                        SenPrice1B += ScenarioAnalysis."Profit/Loss on Scenario 1B";
                        SenPrice2A += ScenarioAnalysis."Profit/Loss on Scenario 2A";
                        SenPrice2B += ScenarioAnalysis."Profit/Loss on Scenario 2B";
                        scenario3 += ScenarioAnalysis."Profit/Loss on Scenario 3";
                        scenario4 += ScenarioAnalysis."Profit/Loss on Scenario 4";
                ...
                // Clear(scenario4);
                // ScenarioAnalysis.Reset();
                // ScenarioAnalysis.SetRange("CmCode", "Client Level Profit Loss".CmCode);
                // ScenarioAnalysis.SetRange("TmCode", "Client Level Profit Loss".TmCode);
                ...
                if "Client Level Profit Loss".ClientCode = 'OWN' then
                    NetPLSenPrice1A := SenPrice1A
                else
                    NetPLSenPrice1A := "Client Level Profit Loss".Margin + SenPrice1A;


                if "Client Level Profit Loss".ClientCode = 'OWN' then
                    NetPLSenPrice1B := SenPrice1B
                else
                    NetPLSenPrice1B := "Client Level Profit Loss".Margin + SenPrice1B;

                if "Client Level Profit Loss".ClientCode = 'OWN' then
                    NetPLSenPrice2A := SenPrice2A
                else
                    NetPLSenPrice2A := "Client Level Profit Loss".Margin + SenPrice2A;

                if "Client Level Profit Loss".ClientCode = 'OWN' then
                    NetPLSenPrice2B := SenPrice2B
                else
                    NetPLSenPrice2B := "Client Level Profit Loss".Margin + SenPrice2B;

                if "Client Level Profit Loss".ClientCode = 'OWN' then
                    NetPLscenario3 := scenario3
                else
                    NetPLscenario3 := "Client Level Profit Loss".Margin + scenario3;

                if "Client Level Profit Loss".ClientCode = 'OWN' then
                    NetPLscenario4 := scenario4
                else
                    NetPLscenario4 := "Client Level Profit Loss".Margin + scenario4;
                ...
                ExcelBuffer.AddColumn("Client Level Profit Loss".CmCode, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Text);
                ExcelBuffer.AddColumn("Client Level Profit Loss".TmCode, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Number);
                ExcelBuffer.AddColumn("Client Level Profit Loss".ClientCode, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Number);
                ...
                ExcelBuffer.AddColumn("Client Level Profit Loss".Margin, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Number);
                ExcelBuffer.AddColumn(NetPLSenPrice1A, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Number);
                ExcelBuffer.AddColumn(NetPLSenPrice1B, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Number);
                ExcelBuffer.AddColumn(NetPLSenPrice2A, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Number);
                ExcelBuffer.AddColumn(NetPLSenPrice2B, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Number);
                ExcelBuffer.AddColumn(NetPLscenario3, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Number);
                ExcelBuffer.AddColumn(NetPLscenario4, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Number);
                ...
                //Createoutputs("Client Level Profit Loss");
            end;
        }
    }
    ...
    local procedure Createoutputs(clientLevlPL: Record "Client Level Profit Loss")
    // Insert into Client Level P&L OutPut
    var
        PLOutputRec: Record "Client Level P&L OutPut";
        LastSrNo: Integer;
    begin
        //Clear(PLOutputRec);
        PLOutputRec.Init();
        ...
        if PLOutputRec.FindLast() then
            LastSrNo := PLOutputRec."Entry No." + 1
        else
            LastSrNo := 1;

        PLOutputRec."Entry No." := LastSrNo;
        PLOutputRec."CmCode" := "Client Level Profit Loss".CmCode;
        PLOutputRec."TmCode" := "Client Level Profit Loss".TmCode;
        PLOutputRec."ClientCode" := "Client Level Profit Loss".ClientCode;
        PLOutputRec."Profit/Loss Scenario1A" := SenPrice1A;
        PLOutputRec."Profit/Loss Scenario1B" := SenPrice1B;
        PLOutputRec."Profit/Loss Scenario2A" := SenPrice2A;
        PLOutputRec."Profit/Loss Scenario2B" := SenPrice2B;
        PLOutputRec."Profit/Loss Scenario3" := scenario3;
        PLOutputRec."Profit/Loss Scenario4" := scenario4;
        PLOutputRec."Margin" := "Client Level Profit Loss".Margin;
        PLOutputRec."NetLoss Scenario1A" := NetPLSenPrice1A;
        PLOutputRec."NetLoss Scenario1B" := NetPLSenPrice1B;
        PLOutputRec."NetLoss Scenario2A" := NetPLSenPrice2A;
        PLOutputRec."NetLoss Scenario2B" := NetPLSenPrice2B;
        PLOutputRec."NetLoss Scenario3" := NetPLscenario3;
        PLOutputRec."NetLoss Scenario4" := NetPLscenario4;
        PLOutputRec."NetLoss (-) OnScenario1A" := NetLLossenPrice1A;
        PLOutputRec."NetLoss (-) OnScenario1B" := NetLLossSenPrice1B;
        PLOutputRec."NetLoss (-) OnScenario2A" := NetLLossSenPrice2A;
        PLOutputRec."NetLoss (-) OnScenario2B" := NetLLossSenPrice2B;
        PLOutputRec."NetLoss (-) OnScenario3" := NetLLossscenario3;
        PLOutputRec."NetLoss (-) OnScenario4" := NetLLossscenario4;
        if not PLOutputRec.Insert(true) then PLOutputRec.Modify();
    end;
